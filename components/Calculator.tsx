...
export default function Calculator() {
    ...
    const calculatePrice = (e: React.FormEvent) => {
        ...
        let finalText = "";
        let soundType: "cash" | "error" | "success" | "siren" | "click" = "click";

        // 1. Opposition Logic (Priority: High Risk)
        if (partyLevel === "opposition") {
            if (respectLevel === "mastan" || voiceLevel === "high") {
                finalText = "⚠️ পুলিশ আপনাকে খুঁজছে! ভোট বাদ দিয়ে পালান দ্রুত! (মামলা ১০টা রেডি)";
                soundType = "siren";
            } else {
                finalText = "ভোট কেন্দ্রে যাওয়ার দরকার নাই। আপনার ভোট আগেই দেওয়া হয়ে গেছে (অন্য কেউ দিয়েছে)।";
                soundType = "error";
            }
        }
        // 2. Government Logic (Priority: VIP)
        else if (partyLevel === "government") {
            if (respectLevel === "future_member") {
                finalText = "আরে নেতা! আপনি কেন ভোট বেচবেন? আপনি তো নমিনেশন কিনবেন! (বাজেট: ৫ কোটি)";
                soundType = "cash";
            } else if (respectLevel === "uncle") {
                finalText = "চাচা, আপনি শুধু দোয়া কইরেন। আপনার ভোট আমরাই দিয়ে দিবো নে।";
                soundType = "success";
            } else {
                finalText = "আপনি কি কর্মী? বিরিয়ানি আর ২০০ টাকা নিয়ে লাইনে দাঁড়ান।";
                soundType = "click";
            }
        }
        // 3. Student Logic (Priority: Respect)
        else if (partyLevel === "student") {
            if (voiceLevel === "high") {
                finalText = "দেশ গড়ার কারিগর! আপনার ভোট বেচা নিষেধ। সিস্টেম বদলাতে হবে! 💪";
                soundType = "success";
            } else {
                finalText = "ভাই, পড়াশোনা করুন। ভোট নিয়ে টেনশন নাই।";
                soundType = "click";
            }
        }
        // 4. General Public (Party: None)
        else {
            // Mastan
            if (respectLevel === "mastan") {
                finalText = "এলাকার ভাই! আপনার ভোট লাগবে না, আপনি বরং ৫টা কেন্দ্র দখল করে দেন। (রেট: ৫০ হাজার)";
                soundType = "siren";
            }
            // Uncle / Aunty type
            else if (respectLevel === "uncle") {
                if (voiceLevel === "high") {
                    finalText = "চায়ের দোকানের স্পিকার! গলার জোর আছে কিন্তু ভোটের দাম ১ প্লেট কাচ্চি।";
                    soundType = "click";
                } else {
                    finalText = "মুরুব্বি, বাসায় রেস্ট নেন। রোদে গেলে শরীর খারাপ করবে।";
                    soundType = "error";
                }
            }
            // Future Member wannabe
            else if (respectLevel === "future_member") {
                finalText = "স্বপ্ন দেখেন মেম্বার হওয়ার? আগে ২ লাখ টাকা খরচ করে পোস্টার লাগান।";
                soundType = "error";
            }
            // Unknown / Average
            else {
                if (voiceLevel === "low") {
                    finalText = "আপনার অস্তিত্বই নাই। ভোটার লিস্টে নাম আছে তো?";
                    soundType = "error";
                } else {
                    finalText = "সাধারণ পাবলিক = ১ প্যাকেট তেহারি + ১টা কোক। (নিয়ে খুশি থাকুন)";
                    soundType = "cash";
                }
            }
        }
        ...
        // Delay result and sound for effect
        setTimeout(() => {
            playSound(soundType);
            setShowModal(true);
        }, 600);
